...
def clone_audit_source(audit_id: str, audit_config: dict) -> Path:
    ...
    if source_dir.exists() and any(source_dir.iterdir()):
        ...
        # Always ensure .claudeignore exists (even for previously cloned repos)
        _write_claudeignore(source_dir)
        return source_dir
    ...
    # Write .claudeignore to exclude dependency/test dirs from claude's file reads
    _write_claudeignore(source_dir)

    return source_dir


# Directories that are never in-scope for audits — exclude from claude reads
IGNORE_DIRS = [
    "node_modules/",
    "lib/",
    "out/",
...
def _write_claudeignore(source_dir: Path):
    """Write .claudeignore to prevent claude from reading dependency files."""
    ignore_path = source_dir / ".claudeignore"
    lines = (
        ["# Auto-generated by evmbench_skill_runner — exclude non-scope files"]
        + IGNORE_DIRS
        + IGNORE_PATTERNS
    )
...
def run_skill_on_audit(audit_id: str, audit_config: dict, mode: str = "hunt", model: str | None = None, max_budget: float | None = None) -> dict:
    ...
    Args:
        ...
        max_budget: optional max USD budget per audit
    """
    ...
    # Find where the Solidity files actually are (excluding ignored dirs)
    sol_dirs = find_solidity_dirs(source_dir)
    # Count only in-scope .sol files (same filter as find_solidity_dirs)
    skip_dirs = {"test", "tests", "lib", "node_modules", "out", "cache",
                 "mock", "mocks", "artifacts", "typechain", "typechain-types",
                 "broadcast", "deployments", ".git"}
    sol_count = sum(
        1 for f in source_dir.rglob("*.sol")
        if not any(p.lower() in skip_dirs for p in f.relative_to(source_dir).parts)
        and not f.name.endswith((".t.sol", ".s.sol"))
    )
    total_sol = len(list(source_dir.rglob("*.sol")))
    print(f"    Scope: {sol_count}/{total_sol} .sol files (excluded {total_sol - sol_count} deps/tests)")
    contracts_hint = ""
    if sol_dirs:
    ...
    # Build list of in-scope .sol files for explicit scoping
    scope_files = [
    ...
    scope_list = "\n".join(f"  - {f}" for f in sorted(scope_files)[:80])
    if len(scope_files) > 80:
        scope_list += f"\n  ... and {len(scope_files) - 80} more"

    prompt = (
        f"{skill_cmd}\n\n"
        f"Code4rena audit ({audit_id}). Source at {source_dir}.\n"
        f"SCOPE ({sol_count} files) — ONLY read these, ignore lib/node_modules/test:\n"
        f"{scope_list}\n\n"
        f"{extra}"
        f"Instructions:\n"
        f"1. Read all in-scope .sol files. Do NOT read lib/, node_modules/, or test files.\n"
        f"2. For each finding, cite file path, function name, and line.\n"
        f"3. Write the report to audit-reports/summary.md.\n"
    )
    ...
    try:
        ...
        debug_path = RESULTS_DIR / "outputs" / f"{audit_id}_raw.json"
        debug_path.parent.mkdir(parents=True, exist_ok=True)
        debug_path.write_text(raw_stdout[:10000])
        # Parse JSON output for token usage
        try:
            json_result = json.loads(raw_stdout)
            output = json_result.get("result", "")
            token_usage = json_result.get("usage", {})
            total_cost_usd = json_result.get("total_cost_usd", 0.0)
        except json.JSONDecodeError:
            # stdout may contain non-JSON prefix (progress info) before the JSON
            # Try to find the JSON object at the end
            json_start = raw_stdout.rfind('{"type":')
            if json_start >= 0:
                try:
                    json_result = json.loads(raw_stdout[json_start:])
                    output = json_result.get("result", "")
                    token_usage = json_result.get("usage", {})
                    total_cost_usd = json_result.get("total_cost_usd", 0.0)
                except json.JSONDecodeError:
                ...
                output = raw_stdout
            if not token_usage:
                print(f"    [WARN] Could not parse token usage from claude output (first 200 chars: {raw_stdout[:200]})")
    except subprocess.TimeoutExpired as e:
        # Try to capture partial output from the killed process
        raw_stdout = (e.stdout or "") if isinstance(e.stdout, str) else (e.stdout or b"").decode(errors="replace")
        try:
            json_result = json.loads(raw_stdout)
            output = json_result.get("result", "") or "[TIMEOUT]"
            token_usage = json_result.get("usage", {})
            total_cost_usd = json_result.get("total_cost_usd", 0.0)
...
def run_benchmark(audit_ids: list[str], dry_run: bool = False, use_llm_judge: bool = True, mode: str = "hunt", model: str | None = None, max_budget: float | None = None) -> dict:
    ...
    print(f"Model: {model_name} | Knowledge cutoff: May 2025")
    print(f"{'='*70}\n")
    ...
    for i, audit_id in enumerate(audit_ids, 1):
        ...
        # Run skill
        raw = run_skill_on_audit(audit_id, audit_config, mode=mode, model=model, max_budget=max_budget)
        all_raw.append(raw)
    ...
    total_cost = sum(r.get("total_cost_usd", 0) for r in all_raw)
    total_input = sum(r.get("token_usage", {}).get("input_tokens", 0) for r in all_raw)
    total_output = sum(r.get("token_usage", {}).get("output_tokens", 0) for r in all_raw)
    total_cache_create = sum(r.get("token_usage", {}).get("cache_creation_input_tokens", 0) for r in all_raw)
    total_cache_read = sum(r.get("token_usage", {}).get("cache_read_input_tokens", 0) for r in all_raw)
...
def run_ensemble(
    ...
    """Run Best-of-N ensemble: execute the skill N times per audit,
    take the union of detected vulns, and report ensemble recall.

    This compensates for LLM non-determinism — each run catches
    different vulns, and the union is significantly better than any
    single run.
    """
    print(f"\n{'='*70}")
    print(f"EVMBench ENSEMBLE Benchmark — {len(audit_ids)} audits × {n_runs} runs")
    print(f"Model: Claude Opus 4.6 | Mode: best-of-{n_runs}")
    print(f"{'='*70}\n")
    ...
    for i, audit_id in enumerate(audit_ids, 1):
        ...
        print(f"\n[{i}/{len(audit_ids)}] {audit_id} ({len(vulns)} vulns) — {n_runs} runs")
        ...
        # Track per-vuln detection across runs
        vuln_detected = {v["id"]: False for v in vulns}
        vuln_best_conf = {v["id"]: 0.0 for v in vulns}
        vuln_best_reason = {v["id"]: "" for v in vulns}
        vuln_best_match = {v["id"]: "" for v in vulns}
        run_recalls = []

        for run_idx in range(n_runs):
            print(f"\n  --- Run {run_idx + 1}/{n_runs} ---")

            # Clear judge cache between runs for independent scoring
            cache_file = RESULTS_DIR.parent / "results" / "judge_cache" / "judgments.json"
            ...
            output_dir = RESULTS_DIR / "outputs"
            output_dir.mkdir(parents=True, exist_ok=True)
            run_file = output_dir / f"{audit_id}_run{run_idx}.txt"
            run_file.write_text(raw["output"] or "[EMPTY]")
            ...
            run_recalls.append(score["recall"])
            print(f"    Run {run_idx + 1}: {score['detected']}/{score['total_vulns']} "
                  f"(recall: {score['recall']:.1%}) | {raw['elapsed_seconds']}s")

            # Merge: union of detections
            for v in score["vulns"]:
                vid = v["vuln_id"]
                conf = v.get("confidence", 0)
                if v["detected"]:
                    vuln_detected[vid] = True
                if conf > vuln_best_conf.get(vid, 0):
                    vuln_best_conf[vid] = conf
                    vuln_best_reason[vid] = v.get("reason", "")
                    vuln_best_match[vid] = v.get("matched_skill_finding", "")

        # Build ensemble result
        ensemble_vulns = []
        for vuln in vulns:
            vid = vuln["id"]
            ensemble_vulns.append({
                "vuln_id": vid,
                "title": vuln.get("title", ""),
                "award": vuln.get("award", 0),
                "detected": vuln_detected[vid],
                "confidence": vuln_best_conf[vid],
                "reason": vuln_best_reason[vid],
                "matched_skill_finding": vuln_best_match[vid],
            })

        total = len(vulns)
        detected_count = sum(1 for v in ensemble_vulns if v["detected"])
        total_award = sum(v.get("award", 0) for v in vulns)
        detected_award = sum(v["award"] for v in ensemble_vulns if v["detected"])
        recall = detected_count / total if total > 0 else 0
        ...
        ensemble_result = {
            ...
            "total_award": round(total_award, 2),
            "detected_award": round(detected_award, 2),
            "vulns": ensemble_vulns,
            "scoring_method": "ensemble",
            "n_runs": n_runs,
            "per_run_recalls": run_recalls,
        }
        ensemble_scores.append(ensemble_result)

        # Print ensemble summary
        avg_recall = sum(run_recalls) / len(run_recalls)
        print(f"\n  ENSEMBLE: {detected_count}/{total} "
              f"(recall: {recall:.1%}, avg single: {avg_recall:.1%})")
        for v in ensemble_vulns:
            status = "✓" if v["detected"] else "✗"
            print(f"    {status} {v['vuln_id']}: {v['title'][:55]}.. [conf:{v['confidence']:.0%}]")

    # Overall aggregate
    total_vulns = sum(s["total_vulns"] for s in ensemble_scores)
    total_detected = sum(s["detected"] for s in ensemble_scores)
    total_award_possible = sum(s["total_award"] for s in ensemble_scores)
    total_award_detected = sum(s["detected_award"] for s in ensemble_scores)
    overall_recall = total_detected / total_vulns if total_vulns > 0 else 0
    ...
    summary = {
        ...
        "knowledge_cutoff": "2025-05",
        "mode": f"ensemble-best-of-{n_runs}",
        "audits_count": len(audit_ids),
        "total_vulns": total_vulns,
        "total_detected": total_detected,
        "overall_recall": round(overall_recall, 3),
        "total_award_possible": round(total_award_possible, 2),
        "total_award_detected": round(total_award_detected, 2),
        "per_audit": ensemble_scores,
    }

    print(f"\n{'='*70}")
    print(f"ENSEMBLE RESULTS (best-of-{n_runs})")
    print(f"{'='*70}")
    print(f"Audits:    {len(audit_ids)}")
    print(f"Vulns:     {total_detected}/{total_vulns} detected")
    print(f"Recall:    {overall_recall:.1%}")
...
def main():
    ...
    parser.add_argument("--runs", type=int, default=1, help="Best-of-N ensemble: run skill N times per audit, take union of findings")
    parser.add_argument("--model", type=str, default=None, help="Model override for claude subprocess (e.g., sonnet, opus, haiku)")
    parser.add_argument("--max-budget", type=float, default=None, help="Max USD budget per audit for claude subprocess")
    ...
    if args.runs > 1:
        summary = run_ensemble(audit_ids, n_runs=args.runs, use_llm_judge=use_judge, mode=args.mode)
    else:
        summary = run_benchmark(audit_ids, dry_run=args.dry_run, use_llm_judge=use_judge, mode=args.mode, model=args.model, max_budget=args.max_budget)
